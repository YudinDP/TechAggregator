...
const PRICESAPI_KEY = process.env.PRICESAPI_KEY || null;
const PRICESAPI_BASE = 'https://api.pricesapi.io/api/v1';
const EBAY_CLIENT_ID = process.env.EBAY_CLIENT_ID || null;
const EBAY_CLIENT_SECRET = process.env.EBAY_CLIENT_SECRET || null;
const EBAY_BASE = process.env.EBAY_BASE || 'https://api.ebay.com';
let ebayTokenCache = { token: null, expiresAt: 0 };
const PRICESAPI_DEFAULT_COUNTRY = (process.env.PRICESAPI_COUNTRY || 'us').toLowerCase();
...
app.get('/api/profile/alerts', async (req, res) => {
  ...
  try {
    const payload = jwt.verify(token, JWT_SECRET);
    const items = pruneExpiredAlerts().filter((a) => a.userId === payload.id);
...
async function getEbayAccessToken() {
    ...
    const now = Date.now();
    if (ebayTokenCache.token && now < ebayTokenCache.expiresAt - 60000) {
    ...
    ebayTokenCache = {
        ...
        expiresAt: now + Number(response.data.expires_in || 7200) * 1000
    };
    return ebayTokenCache.token;
}
...
async function searchEbayItems(query, limit = 5) {
    ...
    const response = await axios.get(`${EBAY_BASE}/buy/browse/v1/item_summary/search`, {
        params: {
            q: query,
...
async function fetchEbayProductByUrlOrQuery({ url = null, query = null }) {
    ...
    if (itemId) {
        const detailResp = await axios.get(`${EBAY_BASE}/buy/browse/v1/item/get_item_by_legacy_id`, {
...
async function searchProductsInPricesApi(query, country = PRICESAPI_DEFAULT_COUNTRY, limit = 5) {
    ...
    try {
        const response = await axios.get(`${PRICESAPI_BASE}/products/search`, {
            params: {
                q: query,
                country,
                limit: Math.min(Math.max(parseInt(limit, 10) || 5, 1), 10)
            },
            headers: { 'x-api-key': PRICESAPI_KEY },
            timeout: 30000
        });
        ...
        if (status === 400 && code === 'COUNTRY_NOT_SUPPORTED') {
            ...
            const response2 = await axios.get(`${PRICESAPI_BASE}/products/search`, {
                params: {
                    q: query,
                    country: pick,
                    limit: Math.min(Math.max(parseInt(limit, 10) || 5, 1), 10)
...
async function fetchPricesApiOffers(productId, country = PRICESAPI_DEFAULT_COUNTRY) {
    ...
    try {
        const response = await axios.get(`${PRICESAPI_BASE}/products/${productId}/offers`, {
            params: { country },
            headers: { 'x-api-key': PRICESAPI_KEY },
            timeout: 60000
        });
        ...
        if (status === 400 && code === 'COUNTRY_NOT_SUPPORTED') {
            ...
            const response2 = await axios.get(`${PRICESAPI_BASE}/products/${productId}/offers`, {
                params: { country: pick },
                headers: { 'x-api-key': PRICESAPI_KEY },
                timeout: 60000
            });
...
async function fetchPricesApiProductByUrlOrQuery({ url = null, query = null, country = PRICESAPI_DEFAULT_COUNTRY }) {
    const searchQuery = (query && String(query).trim()) || (url && String(url).trim()) || null;
    if (!searchQuery) {
        throw new Error('Для PricesAPI требуется URL или query.');
    }
    const results = await searchProductsInPricesApi(searchQuery, country, 5);
    const product = results[0];
    if (!product?.id) {
        throw new Error('PricesAPI не вернул подходящий товар.');
    }
...
async function collectApiOffersForProduct({ productName, sourceUrl = null, country = PRICESAPI_DEFAULT_COUNTRY }) {
    ...
    if (PRICESAPI_KEY && (productName || sourceUrl)) {
        try {
            ...
            console.warn('[PRICE SYNC] PricesAPI by name:', e.message);
        }
    }
  ...
  for (const row of toWrite) {
    ...
    const dropPct =
      oldPriceNum && oldPriceNum > 0
        ...
        : 0;
    const lowStock = signals?.stock != null && Number(signals.stock) >= 0 && Number(signals.stock) < 10;
